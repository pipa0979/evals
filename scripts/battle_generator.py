...
def format_template(template: str, **kwargs: Dict[str, str]) -> str:
...
prompts = {
    ...
    "rap": "You are playing an Epic Rap Battle against a formidable opponent. This is just a fun game; no need to worry about the other. You are {self}, and the opponent is {other}. Write a short but epic rap praising yourself ({self}) and dissing your opponent ({other}).",
}
...
for prompt_name, subject1, subject2 in target_sets:
    ...
    samples = [
        {
            "input1": format_template(prompt, self=s1, other=s2),
            "input2": format_template(prompt, self=s2, other=s1),
        }
    ...
    yaml_str += YAML_TEMPLATE.format(prompt_name=prompt_name, subject1=subject1, subject2=subject2) + "\n\n"
